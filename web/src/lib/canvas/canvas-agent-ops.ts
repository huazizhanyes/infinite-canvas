...
import { appendValidCanvasConnections } from "@/lib/canvas/canvas-connection-rules";
...
export function applyCanvasAgentOps(snapshot: CanvasAgentSnapshot, ops?: CanvasAgentOp[]) {
    ...
    (Array.isArray(ops) ? ops : []).forEach((op, index) => {
        ...
        if (op.type === "connect_nodes") {
            ...
            connections = appendValidCanvasConnections(nodes, connections, [{ id: op.id || nanoid(), fromNodeId: op.fromNodeId, toNodeId: op.toNodeId }]);
        }
